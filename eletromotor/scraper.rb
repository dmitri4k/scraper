...
def scrape_and_convert(url)
  begin
    ...
    page.css('ul.p-nav-3 > li > a').each do |link| # Links generation
      article_url = $site + link['href']
      article_title = link.text.strip
      article_name = article_url.split('/').last.split('.').take(2).join('.')

      html_article_content = URI.open(article_url).read
      article_page = Nokogiri::HTML(html_article_content)
      article_page_clear = article_page.css('.center')
      ...
      article_page_clear.css('img').each do |img| # Image download
...
$site = 'http://ru.about-motors.com/motorcontrol/motor'
url = $site + ''
